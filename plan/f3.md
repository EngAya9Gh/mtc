...
### 15.3 تفاصيل شاشة الخريطة وتأكيد الموقع (Map Logic & Location Confirm)

شاشة الخريطة (`ListMapFragment`) تحتوي على تفاصيل برمجية دقيقة يجب محاكاتها في فلاتر:

**أ. اللوجيك الخاص بالواجهة (UI/UX):**
- **المسافة (Distance):** التطبيق **لا** يستخدم API جوجل لحساب مسار القيادة (لتقليل التكلفة). بل يقوم بحساب "المسافة بخط مستقيم" (Straight-line Distance) محلياً بين إحداثيات السائق الحالية وإحداثيات العميل (`location.distanceTo()`)، ويعرضها بصيغة `KM`.
- **مدة الرحلة (Time for Journey):** في كود الجافا الحالي لا يوجد حسبة ديناميكية للوقت! يتم إخفاء النص أو إعطاؤه قيمة ثابتة `0`.
- **الملاحة الخارجية (Navigation):** لا يوفر التطبيق ملاحة Turn-by-Turn داخلية. **عند النقر على دبوس موقع العميل (Pin)** أو زر الملاحة، يقوم التطبيق بتشغيل `Intent` لفتح تطبيق **خرائط جوجل الخارجي** عبر الرابط: `http://maps.google.com/maps?daddr={lat},{lng}`.
- **عرض الـ OTP:** إذا كانت المهمة تحتوي على كود `otp` في بياناتها، يظهر الكود للسائق مباشرة على شاشة الخريطة في الأعلى `"Please share OTP: 1234"`.

**ب. تسلسل الأزرار والـ APIs:**
1. **زر الوصول (Reach Location):**
   - يضغط عليه السائق عند وصوله لموقع العميل.
   - يستدعي API: `driver/task/fromlocation/confirm` (POST).
   - عند نجاحه: يُصبح هذا الزر باهتاً (Disabled)، ويتم تفعيل الزر التالي.
2. **زر بدء المهمة / مسح العينات (Start Task / Scan Samples):**
   - **للمهام العادية:** يضغط عليه السائق للانتقال إلى شاشة تحديد أنواع العينات، ويقوم التطبيق في الكواليس بإرسال الموقع عبر API `samples/location`.
   - **لمهام التبادل (Swap):** يستدعي API `driver/task/start` (POST).
